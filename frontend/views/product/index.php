...
/* @var $featuredProducts common\models\Product[] */
            ...
            <div class="card card-primary card-outline">
                ...
                <div class="card-body p-0">
                    <div class="list-group category-list">
                        <a href="<?= Url::to(['/product']) ?>" class="list-group-item list-group-item-action active">
                            <i class="fas fa-th"></i> Tất cả sản phẩm
                        </a>
                        <?php foreach ($categories as $category): ?>
                            <a href="<?= Url::to(['/product/by-category', 'id' => $category->id]) ?>" class="list-group-item list-group-item-action">
                                <?php if ($category->image): ?>
                                    <img src="<?= Yii::$app->urlManager->createUrl('/' . $category->image) ?>" class="category-icon" alt="<?= Html::encode($category->name) ?>">
                                <?php else: ?>
                                    <i class="fas fa-folder"></i>
                                <?php endif; ?>
                                <?= Html::encode($category->name) ?>
                                <span class="badge badge-primary badge-pill"><?= $category->getProducts()->count() ?></span>
                            </a>
                        <?php endforeach; ?>
                    </div>
                </div>
            ...
            <div class="card card-primary card-outline mb-3">
                <div class="card-body">
                    <div class="input-group">
                        <input type="text" class="form-control form-control-lg" id="product-search-input" placeholder="Tìm theo tên, mã hoặc mã vạch sản phẩm...">
                        <div class="input-group-append">
                            <button class="btn btn-primary btn-lg" type="button" id="product-search-button">
                                <i class="fas fa-search"></i> Tìm kiếm
                            </button>
                            <button class="btn btn-info btn-lg" type="button" id="product-barcode-button">
...
<div class="modal fade" id="barcode-scanner-modal" tabindex="-1" role="dialog" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label for="barcode-input">Mã vạch sản phẩm</label>
                    <input type="text" class="form-control form-control-lg" id="barcode-input" placeholder="Nhập hoặc quét mã vạch" autofocus>
                </div>
...
function searchProducts() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // Clear and rebuild grid view
            var gridHtml = '';
            var listHtml = '';
            
            if (products.length === 0) {
                gridHtml = '<div class="col-12"><div class="alert alert-info"><i class="fas fa-info-circle"></i> Không tìm thấy sản phẩm phù hợp.</div></div>';
                listHtml = '<tr><td colspan="6" class="text-center">Không tìm thấy sản phẩm phù hợp.</td></tr>';
            } else {
                // Build grid view
                $.each(products, function(index, product) {
                    ...
                    if (outOfStock) {
                        stockBadge = '<div class="product-badge stock-badge"><span class="badge badge-danger">Hết hàng</span></div>';
                    } else if (product.stock <= 5) { // Assuming min_stock is 5 for simplicity
                        stockBadge = '<div class="product-badge stock-badge"><span class="badge badge-warning">Sắp hết</span></div>';
                    }
                    
                    var comboBadge = product.is_combo ? '<div class="product-badge combo-badge"><span class="badge badge-info">Combo</span></div>' : '';
                    
                    gridHtml += '<div class="col-md-4 col-sm-6 mb-4">' +
                        '<div class="card h-100 product-card ' + (outOfStock ? 'out-of-stock' : '') + '" ' +
                        'data-id="' + product.id + '" ' +
                        'data-code="' + product.code + '" ' +
                        'data-name="' + product.name + '" ' +
                        ...
                        '<i class="fas fa-info-circle"></i>' +
                        '</button>' +
                        '<button type="button" class="btn btn-sm btn-success product-add-btn" data-id="' + product.id + '"' + (outOfStock ? ' disabled' : '') + '>' +
                        '<i class="fas fa-plus"></i>' +
                        '</button>' +
                        '</div>' +
                        '</div>' +
                        ...
                        '<div class="card-footer bg-white">' +
                        '<small class="text-muted">' +
                        '<i class="fas fa-box"></i> Tồn kho: ' + product.stock + ' ' + product.unit +
                        '</small>' +
                        '</div>' +
                        '</div>' +
                        '</div>';
                    ...
                    if (outOfStock) {
                        ...
                        stockStatus = ' <span class="badge badge-warning">Sắp hết</span>';
                    }
                    
                    var comboInfo = product.is_combo ? ' <span class="badge badge-info">Combo</span>' : '';
                    
                    listHtml += '<tr class="' + (outOfStock ? 'table-danger' : '') + '">' +
                        '<td><img src="' + product.image + '" class="img-thumbnail" style="width: 50px; height: 50px; object-fit: cover;" alt="' + product.name + '"></td>' +
                        '<td>' + product.name + comboInfo + '</td>' +
                        '<td>' + product.code + '</td>' +
                        '<td class="text-right">' + product.price_formatted + '</td>' +
                        '<td>' + product.stock + ' ' + product.unit + stockStatus + '</td>' +
                        '<td>' +
                        '<button type="button" class="btn btn-sm btn-info product-info-btn" data-id="' + product.id + '">' +
                        '<i class="fas fa-info-circle"></i>' +
                        '</button> ' +
            ...
            $('.product-grid .row').html(gridHtml);
            $('.product-list tbody').html(listHtml);
        },
        error: function() {
...
// Barcode scanner modal
$('#product-barcode-button').on('click', function() {
...
function searchByBarcode() {
    ...
    $('#barcode-result').html('<div class="text-center"><div class="spinner-border text-primary spinner-border-sm" role="status"></div> Đang tìm kiếm...</div>');
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (products.length === 0) {
                $('#barcode-result').html('<div class="alert alert-danger"><i class="fas fa-exclamation-circle"></i> Không tìm thấy sản phẩm với mã vạch này.</div>');
            } else {
                var product = products[0]; // Assume the first match is the correct one
                
                var html = '<div class="card">';
                html += '<div class="card-body">';
                html += '<div class="row">';
                html += '<div class="col-md-4 text-center">';
                html += '<img src="' + product.image + '" class="img-fluid" alt="' + product.name + '">';
                html += '</div>';
                html += '<div class="col-md-8">';
                html += '<h5>' + product.name + '</h5>';
                html += '<p class="text-muted">Mã: ' + product.code + '</p>';
                html += '<p class="text-primary">' + product.price_formatted + '</p>';
                html += '<p>Tồn kho: ' + product.stock + ' ' + product.unit + '</p>';
                html += '<button type="button" class="btn btn-success product-add-modal-btn" data-id="' + product.id + '" ' + (product.stock <= 0 ? 'disabled' : '') + '>';
                html += '<i class="fas fa-cart-plus"></i> Thêm vào giỏ hàng</button>';
                html += '</div>';
                html += '</div>';
                html += '</div>';
                html += '</div>';
        ...
        error: function() {
            $('#barcode-result').html('<div class="alert alert-danger"><i class="fas fa-exclamation-circle"></i> Đã xảy ra lỗi khi tìm kiếm sản phẩm.</div>');
        }
    });
}

// Add to cart from barcode result
$(document).on('click', '.product-add-modal-btn', function() {
    var productId = $(this).data('id');
